...
如果要进行打包并且分发一个项目或者应用，那么最重要的文件就是 serup.py，这个文件应该在你的项目的根目录中。
...
packages 是一个包含这个项目内所有 packages 的列表。你可以使用 `setuptools.find_pakcages()` 来自动查找。使用 `include` 参数来指定仅仅需要查找的包，还可以使用 `exclude` 参数指定被排除在外的包。
...
尽管 package_data 能够满足大部分的需求，但是仍然有些情况不能满足。例如，你的数据文件在你的包外部，data_files 可以做到这一点。

这个参数的值是一个包含多个(directory, files) 元组的列表。每个元祖表达了需要安装到的文件目录以及数据的来源。directory 必须得是相对路径，这个路径是相对于包的安装路径的，默认是 `sys.prefix` 单用户安装时是 `site.USER_BASE`。files 是相对于 setup.py 所在的目录的相对路径。
...
使用 console_scripts entry points 来指定你的脚本的接口。
...
`-e` 代表 `--editable`，而 `.` 代表当前的工作目录，这个命令代表以编辑模式安装当前目录。这同样会安装 `install_requires` 参数指定的所有依赖，以及 `console_scripts` 参数指定的所有脚本。而所有的依赖则会按照通常的不可编辑模式安装。

有时候，同样有需求以编辑模式安装一些依赖。例如，假设你的项目依赖包 foo 与 bar，但是你想将 bar 作为可编辑模式进行安装，那么你可以编写一个这样的 `requirements.txt` 文件：
